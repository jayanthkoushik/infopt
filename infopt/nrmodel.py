...
class NRModel(BOModel):
    ...
    def __init__(
        ...
        p = np.sum([np.prod(p.shape) for p in net.parameters() if p.requires_grad])
        self.Z_inv = 1./lda * torch.eye(p)
    ...
    def _predict_single(self, x, comp_grads=True):
        ...
        v = torch.matmul(flat_dmdp, torch.matmul(self.Z_inv, flat_dmdp))
